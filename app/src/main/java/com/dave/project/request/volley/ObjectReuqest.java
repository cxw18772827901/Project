...
/**
 * Volley常规请求回调的解析类,传参类型:key - value
 * Created by skindhu on 15/5/12.
 */
public class ObjectReuqest<T> extends Request<T> {
    private static final String TAG = ObjectReuqest.class.getName();
    ...
    public ObjectReuqest(boolean needHead, BaseInput<T> input, Response.Listener<T> successListener, Response.ErrorListener listener) {
...
//                MyDebugUtil.logTest("logout_status","链接为="+mInput.url);
        ...
        try {
            ...
            MyDebugUtil.logTest("base", "json解析异常");
            e.printStackTrace();
            return Response.error(new ParseError(response));
        } catch (UnsupportedEncodingException e) {
            MyDebugUtil.logTest("base", "不支持的编码异常");
            e.printStackTrace();
            return Response.error(new ParseError(response));
        } catch (Exception e) {
            MyDebugUtil.logTest("base", "其他异常");
            e.printStackTrace();
...
//        if (maxAge == 0) maxAge = 60;
    ...
    private static Map<String, String> buildCommonParams() {
        ...
        try {
            String deviceId = URLEncoder.encode(DeviceUtil.getIMEI(), "UTF-8");
